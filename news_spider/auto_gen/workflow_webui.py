            ...
            with publish_col1:
                if st.button("📤 发布到生产", key="publish_module", use_container_width=True):
                    with st.spinner("正在发布模块..."):
                        try:
                            # 创建生产目录
                            prod_dir = Path(current_file_dir + "/../news")
                ...
                with button_col1:
                    if st.button("生成代码", key="gen_article_code", use_container_width=True):
                        if st.session_state.article_content:
                            with st.spinner("正在生成代码..."):
                                try:
                                    _c_article_content = compress_html(st.session_state.article_content) if st.session_state.article_content_type == "html" else st.session_state.article_content
                                    print("orgin_length", len(st.session_state.article_content))
                                    print("_c_article_content_length", len(_c_article_content))
